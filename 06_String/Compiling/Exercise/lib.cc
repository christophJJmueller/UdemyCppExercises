...
std::vector<fs::path> get_source_files_in_dir(const fs::path &dir)
{
    ...
    for (auto it = fs::directory_iterator(dir); it != end; ++it)
    {
        ...
        if (fs::is_regular_file(current_file.path()) &&             //directory_iterator erfasst auch Sub-Directories -> daher Abfrage "is_regular_file"
...
void compile_file(fs::path source_file)
{
    ...
    std::system(command.c_str());                               //std::system erwartet einen const char Pointer -> .c_str()
}

...
void set_vehicle_start_data(VehicleType &vehicle,
                            const VehicleLogData &vehicle_log_data)
{
...
void init_vehicles(std::string_view filepath, NeighborVehiclesType &vehicles)
{
    ...
    set_vehicle_start_data(vehicles.vehicles_center_lane[1],
                           vehicles_log_data[3]);
    set_vehicle_start_data(vehicles.vehicles_right_lane[0],
                           vehicles_log_data[4]);
    set_vehicle_start_data(vehicles.vehicles_right_lane[1],
                           vehicles_log_data[5]);
}
...
void load_cycle(const std::uint32_t cycle, NeighborVehiclesType &vehicles)
{
    vehicles.vehicles_left_lane[0].speed_mps =
        vehicles_log_data[0].speeds_mps[cycle];
    vehicles.vehicles_left_lane[1].speed_mps =
        vehicles_log_data[1].speeds_mps[cycle];
    vehicles.vehicles_center_lane[0].speed_mps =
        vehicles_log_data[2].speeds_mps[cycle];
    vehicles.vehicles_center_lane[1].speed_mps =
        vehicles_log_data[3].speeds_mps[cycle];
    vehicles.vehicles_right_lane[0].speed_mps =
        vehicles_log_data[4].speeds_mps[cycle];
    vehicles.vehicles_right_lane[1].speed_mps =
        vehicles_log_data[5].speeds_mps[cycle];
}
